...
public class Program
{
    ...
    {
        IPipelineMiddleware statusMiddleware = new Pong();
        Server server = new Server();
        IConfiguration config = server.create();
        config.setTimeout(500); //mS
        config.addMiddleware("/status", statusMiddleware);
        config.addMiddleware("/try/it", new FirstModule());
        config.addMiddleware("/try/it", new SecondModule());
